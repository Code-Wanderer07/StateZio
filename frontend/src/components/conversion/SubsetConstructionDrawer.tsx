...
export const SubsetConstructionDrawer: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/75 backdrop-blur-xs animate-in fade-in duration-200 select-none">
      <div className="w-full max-w-2xl bg-cyan-50 dark:bg-slate-950 border-l border-cyan-300 dark:border-cyan-500/30 shadow-2xl flex flex-col h-full overflow-hidden text-slate-900 dark:text-slate-100">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50 dark:bg-slate-900">
          {/* Summary Box */}
          <div className="p-4 rounded-xl bg-cyan-100 dark:bg-slate-900 border border-cyan-300 dark:border-cyan-500/30 text-xs text-slate-700 dark:text-slate-300 space-y-1">
          ...
          <div className="space-y-2">
            ...
            <div className="border border-cyan-300 dark:border-cyan-500/30 rounded-xl overflow-hidden shadow-md">
              <table className="w-full text-left text-xs font-mono">
                <thead className="bg-slate-950 text-slate-600 dark:text-slate-400 uppercase text-[10px] tracking-wider border-b border-cyan-200 dark:border-cyan-500/20">
                  <tr>
          ...
          <div className="space-y-2">
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-cyan-300 dark:border-cyan-500/30 font-mono text-xs space-y-1.5 max-h-[220px] overflow-y-auto">
        ...
        {/* Footer */}
        <div className="p-4 border-t border-cyan-200 dark:border-cyan-500/20 bg-cyan-200 dark:bg-slate-800 flex items-center justify-between">
          <span className="text-[11px] text-slate-600 dark:text-slate-400">
            Click "Render Converted DFA" to load onto main canvas.
          </span>
          <button
            onClick={applyConvertedDFA}
            className="px-4 py-2 bg-cyan-400 hover:bg-cyan-300 text-white dark:text-[#1C1313] rounded-xl text-xs font-bold shadow-md transition-all duration-200 cursor-pointer"
          >
            Apply to Canvas
          </button>
        </div>
